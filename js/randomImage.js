...
//get #pictInfo div
let pictInfos = document.getElementById("pictInfo");
...
if (pictInfos) {
    //Empty pictInfos
    while (pictInfos.firstChild) {
        pictInfos.removeChild(pictInfos.firstChild);
    }
    //Create pictInfos title and body
    let pictTitle = document.createElement('h1');
    let parAuthor = document.createElement('p');
    let parExposur = document.createElement('p');
    //Create text in pictInfos
    let pictTitleContent = document.createTextNode(randomImage.name);
    let parAuthorContent = document.createTextNode(randomImage.author);
    let parExposurContent = document.createTextNode(randomImage.exposure);
    //Insert in pictInfos
    //title
    pictTitle.appendChild(pictTitleContent);
    pictInfos.appendChild(pictTitle);
    //author
    parAuthor.appendChild(parAuthorContent);
    pictInfos.appendChild(parAuthor);
    //exposure
    parExposur.appendChild(parExposurContent);
    pictInfos.appendChild(parExposur);
}